...
var appConfmodel = {
    ...
    homepage: {
        ...
        sites: [[{title: "smartUp Gestures", url: "https://smartup.zimoapps.com/"}]],
        ...
        chrome.contextMenus.removeAll(function () {
            ...
            if (config.ctm.settings.disable) {
                sub.ctm.menuDisable = chrome.contextMenus.create(
                    {contexts: ["all"], title: sub.getI18n("ctm_disable"), type: "checkbox"},
                    function () {}
                );
            }
                ...
                chrome.management.getAll(function (exts) {
                    ...
                    for (var i = 0; i < exts.length; i++) {
                        ...
                        (function (i) {
                            // console.time()
                            getBase64(
                                exts[i].icons ? exts[i].icons[exts[i].icons.length - 1].url : "",
                                exts[i].icons ? exts[i].icons[exts[i].icons.length - 1].size : 32,
                                function (data) {
                ...
                switch (message.value.actionType) {
                    ...
                    case "disable":
                        if (
                            ...
                            (config.apps.extmgm.always && config.apps.extmgm.always.contains(message.value.id))
                        ) {
                            break;
                        }
